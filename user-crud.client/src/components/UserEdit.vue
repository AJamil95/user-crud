...
const save = async () => {
  await users.update(id, model.value)
  await store.fetch()
  router.push('/')
}
</script>
...
<template>
  <v-container v-if="model">
    <v-card elevation="2">
      ...
      <v-card-text>
        <v-form>
          <v-row>
            ...
            <v-col cols="12" md="6">
              <v-select :items="store.nationalities" v-model="model.nationality" label="Nationalidad" />
            </v-col>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn color="primary" @click="save">Editar</v-btn>
